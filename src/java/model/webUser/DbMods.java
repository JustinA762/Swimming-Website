...
public class DbMods {
    ...
    public static StringData findForLogon(DbConn dbc, String email, String password)
    {
        StringData sd = new StringData();
        try {
            String sql = "SELECT web_user_id, user_email, user_password, membership_fee, birthday, image, "
            ...
            if (results.next()) { // id is unique, one or zero records expected in result set
                ...
                sd.errorMsg = "Email or Password Not Found.";
            }
            results.close();
            stmt.close();
        } catch (Exception e) {
            sd.errorMsg = "Exception thrown in DbMods.findForLogon(): " + e.getMessage();
        }
        return sd;
    }
